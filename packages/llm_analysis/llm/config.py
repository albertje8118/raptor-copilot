...
def _get_best_thinking_model() -> Optional["ModelConfig"]:
    ...
    for model_entry in models:
        if not isinstance(model_entry, dict):
            logger.debug(
                f"Skipping malformed model entry (not a dict): {type(model_entry)}"
            )
            continue
        ...
        try:
            ...
            logger.debug(
                f"Error processing model entry {model_entry.get('model', 'unknown')}: {e}"
            )
            continue

    if best_model:
        logger.info(
            f"Auto-selected thinking model: {best_model.provider}/{best_model.model_name} (score: {best_score})"
        )
...
def _get_default_primary_model() -> Optional["ModelConfig"]:
    ...
    if thinking_model and thinking_model.api_key:
        logger.info(
            f"Using automatic thinking model: {thinking_model.provider}/{thinking_model.model_name}"
        )
    ...
    if os.getenv("ANTHROPIC_API_KEY"):
        ...
        return ModelConfig(
            ...
            temperature=0.7,
            cost_per_1k_tokens=(costs.get("input", 0.015) + costs.get("output", 0.075))
            / 2,
        )
    ...
    for provider_name, env_var in [
        ...
        if api_key:
            ...
            costs = MODEL_COSTS.get(default_model, {})
            avg_cost = (
                (costs.get("input", 0.005) + costs.get("output", 0.005)) / 2
                if costs
                else 0.002
            )
            return ModelConfig(
        ...
        for model_name in ["claude-opus-4-6", "claude-sonnet-4-6"]:
            ...
            fallbacks.append(
                ModelConfig(
                    ...
                    temperature=0.7,
                    cost_per_1k_tokens=(
                        costs.get("input", 0.003) + costs.get("output", 0.015)
                    )
                    / 2,
                )
            )
        ...
        for model_name in ["gpt-5.4", "gpt-5.2"]:
            ...
            fallbacks.append(
                ModelConfig(
                    ...
                    temperature=0.7,
                    cost_per_1k_tokens=(
                        costs.get("input", 0.006) + costs.get("output", 0.030)
                    )
                    / 2,
                )
            )
        ...
        for model_name in ["gemini-2.5-pro", "gemini-2.5-flash"]:
            ...
            fallbacks.append(
                ModelConfig(
                    ...
                    temperature=0.7,
                    cost_per_1k_tokens=(
                        costs.get("input", 0.002) + costs.get("output", 0.010)
                    )
                    / 2,
                )
            )
...
def resolve_model_roles(
    ...
    analysis_model = (
        analysis[0] if analysis else (all_models[0] if all_models else None)
    )
...
class ModelConfig:
    ...
    cost_per_1k_tokens: float = (
        0.0  # Fallback rate — used only when model not in MODEL_COSTS
    )
    enabled: bool = True
...
class LLMConfig:
    ...
    primary_model: Optional[ModelConfig] = field(
        default_factory=_get_default_primary_model
    )

    # Fallback models (in priority order)
    fallback_models: List[ModelConfig] = field(
        default_factory=_get_default_fallback_models
    )
